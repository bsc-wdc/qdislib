...
def circuit_cutting(
    ...
    if verbose:
        print("Gate where to cut: ", numgates)

    sub_circuit_1_dimension = circuit.queue[numgates - 1].target_qubits[0] + 1
    if verbose:
        print("Sub circuit 1 num qubits: ", sub_circuit_1_dimension)

    if verbose:
        print("Observables to measure: ", observables)

    num_z_sub1 = observables[: (sub_circuit_1_dimension - 1)]
    if verbose:
        print("Observables circuit 1: ", num_z_sub1)

    num_z_sub2 = observables[(sub_circuit_1_dimension - 1) :]
    if verbose:
        print("Observables sub circuit 2: ", num_z_sub2)
...
def simulation(
    observables, qubit, circuit_1, circuit_2=None, shots=30000, verbose=False
):
