            ...
            if gate in target_gates:
                if type(gate) == gates.CNOT:
                    ...
                    if len(combination[idx]) > 2:
                        circuit1.queue[index] = combination[idx][0](control_qubit, np.pi/2)
                        circuit1.queue.insert(index+1,combination[idx][1](control_qubit))
                        circuit1.queue.insert(index+2,combination[idx][2](target_qubit,np.pi/2))
                        circuit1.queue.insert(index+3,combination[idx][3](target_qubit))
                    else:
                        circuit1.queue[index] = combination[idx][0](control_qubit, np.pi/2)
                        circuit1.queue.insert(index+1,combination[idx][1](target_qubit, np.pi/2))
...
def gate_reconstruction(type_gates, gates_cut, exp_values):
    ...
    if type_gates == gates.CZ:
        reconstruction = (1/(1j + 1))*(sum(result1) + sum(result2))/2**len(gates_cut)
    elif type_gates == gates.CNOT:
        reconstruction = 1j*np.exp(-1j*np.pi/4)/np.sqrt(2)*(sum(result1) + sum(result2))/2**len(gates_cut)
    print("\n")
