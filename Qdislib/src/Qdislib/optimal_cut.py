...
def build_dag(circuit,dag):
    for gate in circuit.queue:
        ...
        for qubit in gate.qubits:
            for node in reversed(dag.nodes[:-1]):  # Skip the last node since it is the current gate being added
...
@task(returns=list)
def gate_selector(digraph,circuit, max_qubits=None,num_subcirucits=None,draw=False):
    ...
    for key, array in double_gates.items():
        ...
        if max_qubits == None or has_number(result_list,max_qubits):
            if num_subcirucits == None or num_components == num_subcirucits:
                ...
                comp_cost = 2**len(array) * num_components * max(result_list) * (max(num_nodes)-min(num_nodes))
                ...
                if draw: print("Removing gate ", array)
                if draw: print("Number of connected components:", num_components)
                if draw: print("Number of qubits per subcircuit: ",result_list)
                if draw: print("Number of gates per subcircuit: ",num_nodes)
                if draw: print("Computational Cost: ", comp_cost)
                if draw: print_graph(copy_dag)
...
def find_edge_to_split_graph(graph):
    edges_cut = []
    blue_edges = [(edge[0], edge[1]) for edge in graph.edges.data('color') if edge[2] == 'blue']
    red_edges = [(edge[0], edge[1]) for edge in graph.edges.data('color') if edge[2] == 'red']
...
def wire_selector(digraph,circuit,max_qubits,draw=False):
    ...
    for edge in edge_to_remove:
        ...
        if max_qubits == None or has_number(result_list,max_qubits):
            ...
            comp_cost = 10 * max(result_list) * (max(num_nodes)-min(num_nodes))
            computational_cost.append(comp_cost)
            if draw: print(f"Edge {edge} splits the graph into two disconnected components.")
            if draw: print("Number of qubits per subcircuit: ",result_list)
            if draw: print("Number of gates per subcircuit: ",num_nodes)
            if draw: print("Computational Cost: ", comp_cost)
            if draw: print_graph(temp_graph)
